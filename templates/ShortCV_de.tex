...
\VAR{data.current[station].company} & Seit \textsc{\DTMdate{\VAR{data.current[station].startdate}}} \\
...
\BLOCK{ if data.current is not none }
\BLOCK{for station in data.current}
\textbf{\VAR{data.current[station].institution}}, \VAR{data.current[station].location} &  Seit \textsc{\DTMdate{\VAR{data.current[station].date}}} \\
...
\VAR{data.former[station].name} \BLOCK{ if data.former[station].grade }(Note: \VAR{data.former[station].grade})\BLOCK{ endif } & \\
...
% Software Projects
%
\BLOCK{if projects }\BLOCK{set projects = projects['de']}
...
\textbf{Sprachen:} \BLOCK{ for language in data.languages }\VAR{ data.languages[language].name }\BLOCK{ if loop.last is false}, \BLOCK{ endif }\BLOCK{ endfor } \\
\BLOCK{ endif }
...
\textbf{Entwicklungstools:} \BLOCK{ for other in data.development_tools }\VAR{ data.development_tools[other].name }\BLOCK{ if loop.last is false}, \BLOCK{ endif }\BLOCK{ endfor } \\
\BLOCK{ endif }
